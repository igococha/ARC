...
seq.files <- list.files(path=xml.dir,pattern="*.nex", full.names=FALSE)

xmls <- c()
for(f in seq.files) {
  seq.nexus <- read.nexus.data(paste(xml.dir,f,sep=''))
...
if (FALSE) {
for(f in xmls) {
  system(paste("java -jar beastARC_3.jar",f))
}

for(f in xmls) {
  system(paste("java -jar beastARC_3.jar -resume",f))
}
}
